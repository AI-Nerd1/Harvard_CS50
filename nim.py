...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        The update_q_value function takes a state state, an action action, an existing Q value old_q, a current reward reward, and an estimate of future rewards future_rewards,
        and updates the Q-value for the state/action pair according to the Q-learning formula.
        ...
        if (tuple(state), action) not in self.q.keys():
            self.q[(tuple(state), action)] = reward + future_rewards
        else:
    ...
    def best_future_reward(self, state):
        ...
        for action in actions:
            q_values.add(self.get_q_value(state, action))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        The choose_action function should accept a state as input (and optionally an epsilon flag for whether to use the epsilon-greedy algorithm),
        and return an available action in that state.

        - If epsilon is False, your function should behave greedily and return the best possible action available in that state (i.e., the action that has
        the highest Q-value, using 0 if no Q-value is known).
        - If epsilon is True, your function should behave according to the epsilon-greedy algorithm, choosing a random available action with probability
        self.epsilon and otherwise choosing the best action available.
        ...
        q_max = max(actions_q.values())
        for action in actions_q:
            if actions_q[action] == q_max:
                best_action = action
